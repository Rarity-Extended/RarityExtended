...
function	DialogChoices({router, fight, ratEscaped, adventurerWon, expectedLoot, loot}) {
	if (ratEscaped) {
		return (
			<DialogBox
				options={[
					{label: 'THE BIG UGLY RAT HAS ESCAPED', onClick: () => router.push('/adventures/the-cellar')},
				]} />
		);
	}
	if (adventurerWon) {
		if (expectedLoot === 0) {
			return (
				<DialogBox
					options={[
						{label: 'YOU HAVE DEFEATED THE RAT ! UNFORTUNATELY, THERE IS NOTHING TO RECOVER', onClick: () => router.push('/adventures/the-cellar')},
					]} />
			);
		}
		return (
	...
	return (
		<DialogBox
			options={[
				...
				{label: 'ESCAPE', onClick: () => router.push('/adventures/the-cellar')},
			]} />
	);
}
