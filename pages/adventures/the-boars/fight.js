...
function	DialogChoices({router, fight, boarEscaped, adventurerWon, expectedLoot, loot}) {
	if (boarEscaped) {
		return (
			<DialogNoBox
				options={[
					{label: 'THE WILD BOAR HAS ESCAPED', onClick: () => router.push('/adventures/the-boars')},
				]} />
		);
	}
	if (adventurerWon) {
		if (expectedLoot === 0) {
			return (
				<DialogNoBox
					options={[
						{label: 'YOU HAVE DEFEATED THE WILD BOAR ! UNFORTUNATELY, THERE IS NOTHING TO RECOVER', onClick: () => router.push('/adventures/the-boars')},
					]} />
			);
		}
		return (
	...
	return (
		<DialogNoBox
			options={[
				{label: 'FIGHT', onClick: fight},
				{label: 'ESCAPE', onClick: () => router.push('/adventures/the-boars')},
			]} />
	);
}
