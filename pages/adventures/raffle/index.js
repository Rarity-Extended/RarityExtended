...
function	Index({router}) {
	...
	const	[candies, setCandies] = useState(Number(currentAdventurer?.inventory[process.env.LOOT_CANDIES_ADDR]) || 0);
	...
	useEffect(() => {
		(async () => {
			...
			if(timeToEnd.milliseconds() < 1) {
				router.replace('/adventure/raffle/results');
			} else {
	...
	useEffect(() => {
		(async () => {
			...
			const ownedCandies = Number(currentAdventurer?.inventory[process.env.LOOT_CANDIES_ADDR]) || 0;
	...
	async function buyTickets() {
		...
		if(ticketPurchase > 0) {
			...
			currentAdventurer.inventory[process.env.LOOT_CANDIES_ADDR] = String(candies);
			...
			<div className={'max-w-prose w-full relative mt-8 mx-auto px-3 flex flex-col items-center'}>
				...
				{currentAdventurer.level > 3 && <>
					<div className={'mt-24 flex flex-col items-center text-center'}>
						...
						<Button 
							onClick={() => router.push('/adventure/raffle/sacrifice')}
