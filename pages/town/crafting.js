...
function	IconCook() {
	return (
		<svg xmlns={'http://www.w3.org/2000/svg'} viewBox={'0 0 512 512'} className={'w-10 h-10 text-black dark:text-white'}>
	...
	return (
		<section className={'mt-24 md:mt-12 max-w-screen md:max-w-screen-xl mx-auto'}>
			<div className={'flex flex-row items-center space-x-8 mb-10'}>
				<div
					onClick={() => set_tab(0)}
					className={`w-16 h-16 rounded-full border-2 border-black dark:border-white flex justify-center items-center cursor-pointer transition-opacity ${tab === 0 ? '' : 'opacity-10 hover:opacity-80'}`}>
					<IconCook />
				</div>
				<div
					onClick={() => set_tab(1)}
					className={`w-16 h-16 rounded-full border-2 border-black dark:border-white flex justify-center items-center cursor-pointer transition-opacity ${tab === 1 ? '' : 'opacity-10 hover:opacity-80'}`}>
					<IconGoods />
				</div>
			</div>
