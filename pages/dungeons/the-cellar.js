...
function	DialogChoices({router, step, stepAuto, ratEscaped, adventurerWon, expectedLoot, loot}) {
	if (ratEscaped) {
		return (
			<DialogBox
				options={[
					{label: 'THE BIG UGLY RAT HAS ESCAPED', onClick: () => router.push('/town/tavern?tab=the-cellar')},
				]} />
		);
	}
	if (adventurerWon) {
		if (expectedLoot === 0) {
			return (
				<DialogBox
					options={[
						{label: 'YOU HAVE DEFEATED THE RAT ! UNFORTUNATELY, THERE IS NOTHING TO RECOVER', onClick: () => router.push('/town/tavern?tab=the-cellar')},
					]} />
			);
		}
		return (
	...
	return (
		<DialogBox
			options={[
				...
				{label: 'ESCAPE', onClick: () => router.push('/town/tavern?tab=the-cellar')},
			]} />
	);
}
	...
	return (
		<section className={'mt-12 relative'}>
			<div className={`absolute bg-black inset-0 z-10 -top-24 -left-4 -right-4 flex flex-col items-center min-h-screen transition-opacity duration-1000 ${adventurerHealth <= 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
				...
				<Link href={'/town/tavern?tab=the-cellar'}>
