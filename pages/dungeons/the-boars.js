...
function	DialogChoices({router, step, stepAuto, boarEscaped, adventurerWon, expectedLoot, loot}) {
	if (boarEscaped) {
		return (
			<DialogBox
				options={[
					{label: 'THE BIG UGLY RAT HAS ESCAPED', onClick: () => router.push('/countryside/boars')},
				]} />
		);
	}
	if (adventurerWon) {
		if (expectedLoot === 0) {
			return (
				<DialogBox
					options={[
						{label: 'YOU HAVE DEFEATED THE RAT ! UNFORTUNATELY, THERE IS NOTHING TO RECOVER', onClick: () => router.push('/countryside/boars')},
					]} />
			);
		}
		return (
