...
export async function	careOfAll({provider, tokensID}, callback) {
	...
	**********************************************************************/
	try {
		...
		const	transactionResult = await transaction.wait();
		if (transactionResult.status === 1) {
			callback({error: false, data: tokensID});
			toast.dismiss(_toast);
			toast.success('Transaction successful');
		} else {
			toast.dismiss(_toast);
...
export async function	adventureAll({provider, tokensID}, callback) {
	const	[hadToApprove, success] = await approveRarityExtendedCare({provider, steps: 2});
	if (!success) {
		toast.error('Approve reverted');
		callback({error: true, data: undefined});
...
export async function	adventureCellarAll({provider, tokensID}, callback) {
...
export async function	adventureLevelupAll({provider, tokensID}, callback) {
	const	[hadToApprove, success] = await approveRarityExtendedCare({provider, steps: 2});
	if (!success) {
		toast.error('Approve reverted');
		callback({error: true, data: undefined});
...
export async function	adventureClaimGold({provider, tokensID}, callback) {
	const	[hadToApprove, success] = await approveRarityExtendedCare({provider, steps: 2});
	if (!success) {
		toast.error('Approve reverted');
		callback({error: true, data: undefined});
