...
const RowCraftShields = React.memo(function RowCraftShields({recipe, currentAdventurer, inventory, onCraft}) {
		...
		performBatchedUpdates(() => {
			...
			set_materialsToUse(
				requiredMaterials(
					...
					getArmorDifficulty(recipe.armor_bonus),
					_ratSkinAvailable
				)
			);
		});
	...
	React.useLayoutEffect(() => {
		set_difficulty(
			craftSkillCheck(
				currentAdventurer?.skills[5],
				currentAdventurer?.attributes?.intelligence,
				(getArmorDifficulty(recipe.armor_bonus) - materialsToUse / 10)
			)
		);
	// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [materialsToUse]);
				...
				<div className={'flex flex-row col-span-2'}>
					...
					<div className={'flex flex-col ml-4 w-full'}>
						...
						<p className={'text-sm text-plain-60'}>
							{`Armor: ${recipe.armor_bonus}`}
						</p>
						<p className={'text-sm text-plain-60'}>
							{`Spell Failure: ${recipe.spell_failure}%`}
						</p>
					</div>
				</div>
...
export default RowCraftShields;